...
class StatusAndRouteLatencyAndCounterFilterSpec
  ...
  val configurationForNanoseconds: Configuration = Configuration(
    ConfigFactory.parseString(
      """play-prometheus-filters.exclude.paths = ["/test"]
      play-prometheus-filters.metric.resolution = "nanoseconds" """
    )
  )
  ...
  "Filter constructor" should {
    "Add a histogram and count to the prometheus registry" in {
      ...
      new StatusAndRouteLatencyAndCounterFilter(
        collectorRegistry,
        configuration
      )
  ...
  "Apply method" should {
    "Measure the latency and count" in {
      val filter = new StatusAndRouteLatencyAndCounterFilter(
        mock[CollectorRegistry],
        configurationForMilliseconds
      )
      val rh = FakeRequest().withAttrs(
        TypedMap(
          Router.Attrs.HandlerDef -> HandlerDef(
                null,
                null,
                "testController",
                "test",
    ...
    "Milliseconds - Measure the latency and count for an unmatched route" in {
      val filter = new StatusAndRouteLatencyAndCounterFilter(
        mock[CollectorRegistry],
        configurationForMilliseconds
      )
      ...
      latencySample.value mustBe 1.0
      latencySample.labelValues must have size 5
      latencySample.labelValues.get(
        0
      ) mustBe DefaultPlayUnmatchedDefaults.UnmatchedRouteString
      latencySample.labelValues.get(1) mustBe "200"
      latencySample.labelValues.get(
        2
      ) mustBe DefaultPlayUnmatchedDefaults.UnmatchedControllerString
      latencySample.labelValues.get(
        3
      ) mustBe DefaultPlayUnmatchedDefaults.UnmatchedPathString
      latencySample.labelValues.get(
        4
      ) mustBe DefaultPlayUnmatchedDefaults.UnmatchedVerbString
    ...
    "Seconds - Measure the latency and count for an unmatched route" in {
      val filter = new StatusAndRouteLatencyAndCounterFilter(
        mock[CollectorRegistry],
    ...
    "Nanoseconds - Measure the latency and count for an unmatched route" in {
      val filter = new StatusAndRouteLatencyAndCounterFilter(
        mock[CollectorRegistry],
        configurationForNanoseconds
      )
      val rh     = FakeRequest()
      val action = new MockController(stubControllerComponents()).ok
